...
export const getMealBySlugAsAdmin = async (req: Request, res: Response) => {
  const { mealSlug } = req.params;
  const meal = await getMealBySlugService(mealSlug as unknown as string);
...
export const deleteMealAsAdmin = async (req: Request, res: Response) => {
  const { mealSlug } = req.params;
  await deleteMealAsAdminService(mealSlug as unknown as string);
